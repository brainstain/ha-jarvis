...
JSON events over a plain TCP socket, optionally followed by a raw binary
payload when the event carries `payload_length`. Every Wyoming service
answers a `describe` event with an `info` event listing what it can do
(models loaded, languages, etc.) — that's a real protocol round trip, not
just "the port accepts a connection," without us needing to ship an audio
sample through the test suite.
"""
...
def describe(host: str, port: int, timeout: float = 3.0) -> dict:
    """Send a `describe` event and return the decoded `info` response.
